...
type AuthMiddleware struct {
	...
	scopes           []string
	userLoadFunction func(token *oauth2.Token) (string, error)
	oauthConf        *oauth2.Config
}

func NewAuthMiddleware(handler http.Handler, config AuthConfig, scopes []string, userLoadFunction func(token *oauth2.Token) (string, error)) *AuthMiddleware {
	r := &AuthMiddleware{
		Config:           config,
		handler:          handler,
		scopes:           scopes,
...
func (s *AuthMiddleware) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(r.URL.Path, "/auth/google") || strings.HasPrefix(r.URL.Path, "/favicon") {
		...
		if err != nil {
			...
			if err != nil {
				...
				http.Redirect(w, r, "/auth/google/login", 302)
				return
			}
...
func (u *UserSession) GetToken() *oauth2.Token {
...
func (s *AuthMiddleware) googleCallback(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	userData, err := s.userLoadFunction(token)
	if err != nil {
	...
	http.Redirect(w, r, "/auth/google/status", 302)
